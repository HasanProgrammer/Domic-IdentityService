...
public class OtpVerificationCommandHandler(
    IEmailOtpLogCommandRepository otpLogCommandRepository, IDateTime dateTime, 
...
) : ICommandHandler<OtpVerificationCommand, string>
{
    ...
    public async Task<string> HandleAsync(OtpVerificationCommand command, CancellationToken cancellationToken)
    {
        ...
        targetOtpLog.ChangeVerification(dateTime, command.EmailAddress, true, targetOtpLog.UserId, targetOtpLog.CreatedRole);
